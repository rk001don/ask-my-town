...
export function ProductSheet({
  ...
  return (
    <div
      ...
      role="presentation"
    >
      {/* The close button sits centred *above* the sheet rather than tucked
          into its top-right corner -- the Swiggy placement. On a phone the
          top-right corner of a full-width sheet is the hardest point on the
          screen to reach one-handed, and it also sat on top of the photo. */}
      <div className="relative w-full max-w-[560px] pt-14">
        <button
          ref={closeRef}
          onClick={onClose}
          aria-label="Close"
          className="tap-scale absolute left-1/2 top-2 z-10 grid h-10 w-10 -translate-x-1/2 place-items-center rounded-full bg-black/60 text-white backdrop-blur-sm"
        >
          <X className="h-5 w-5" />
        </button>
        <div
          ...
          className="sheet-in flex max-h-[calc(92dvh-3.5rem)] w-full flex-col overflow-hidden rounded-t-3xl bg-[color:var(--bg-elevated)] sm:rounded-3xl"
        >
          <div
            className="grid aspect-[16/10] w-full shrink-0 place-items-center overflow-hidden"
